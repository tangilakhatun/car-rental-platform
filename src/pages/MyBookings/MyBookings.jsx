...
export default function MyBookings() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      // প্রতিটি booking এর জন্য car info fetch
      const bookingsWithCar = await Promise.all(
        res.data.map(async (b) => {
          let carName = b.carName;
          let imageUrl = b.imageUrl;

          if (!carName || !imageUrl) {
            try {
              const carRes = await api.get(`/cars/${b.carId}`, {
                headers: { Authorization: `Bearer ${token}` },
              });
              carName =
                carRes.data.carName ||
                ...
                "No Name";
              imageUrl =
                carRes.data.imageUrl ||
                carRes.data.image ||
                "https://via.placeholder.com/400x300?text=No+Image";
            } catch {
              carName = carName || "No Name";
              imageUrl = imageUrl || "https://via.placeholder.com/400x300?text=No+Image";
            }
          }

          return { ...b, carName, imageUrl };
        })
      );

      setBookings(bookingsWithCar);
    } catch (err) {
      console.error(err);
      Swal.fire(
        "Error",
